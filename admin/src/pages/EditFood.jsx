...
import useUpdateFood from "../store/useUpdateFood";
...
const EditFood = () => {
	...
	useEffect(() => {
		...
		const selectedFood = foods?.filter((food) => food._id === foodId)[0];
		setFormField("id", selectedFood?._id);
		setFormField("foodName", selectedFood?.foodName);
		setFormField("price", selectedFood?.price);
		setFormField("category", selectedFood?.category);
		setFormField("ingredients", selectedFood?.ingredients);
		setFormField("prepTime", selectedFood?.prepTime);
		setFormField("description", selectedFood?.description);
		if (selectedFood?.images?.length > 0) {
			const mapimg = selectedFood?.images?.map((img) => img.url);
			addImages(mapimg, showMsg);
		}
	}, [foodId, navigate]);
