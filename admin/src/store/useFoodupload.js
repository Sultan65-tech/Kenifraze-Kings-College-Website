...
const useFoodUpload = create((set, get) => ({
	...
	submitFoodItem: async (e, showMsg) => {
		...
		if (images.length === 0) {
			showMsg("Please upload at least one food image.", false);
			return;
		}
		for (let item in formData) {
			if (formData[item] === "" || !formData[item]) {
				showMsg(`${item} - is required!`, false);
				return;
			}
		}
		...
		try {
			...
			const res = await axios.post("/api/admin/add-food", payload);
			if (!res?.data.success) {
			...
			set({ success: true });
			get().resetForm();
			showMsg("Menu item uploaded successfully!", true);
		} catch (err) {
